...
static void print_help() {
    cout << "oat_closed_way_filter [OPTIONS] OSMFILE -o OUTPUT\n\n"
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (output_filename.empty() || optind != argc - 1) {
            cerr << "Usage: " << argv[0] << " [OPTIONS] OSMFILE -o OUTPUT\n";
            return exit_code_cmdline_error;
        }
        ...
        copy_if(ways.cbegin(), ways.cend(), output_it, [](const osmium::Way& way) {
