...
class StatsHandler : public osmium::handler::Handler {
    ...
    array<uint32_t, 2001> m_nodes_in_ways_histogram{};

    unordered_map<uint32_t, uint32_t> m_nodes_in_ways;

    vector<uint32_t> m_ways_in_relations;
    vector<uint32_t> m_nodes_in_relations;
        ...
        for (const auto& member : relation.members()) {
            switch (member.type()) {
                ...
                case osmium::item_type::way:
                    ++m_member_ways;
                    ++way_members;
                    if (!strcmp(member.role(), "outer")) {
                        ++m_roles_outer;
                    } else if (!strcmp(member.role(), "inner")) {
                        ++m_roles_inner;
                    } else if (!strcmp(member.role(), "")) {
                        ++m_roles_empty;
                    } else {
                        ++m_roles_other;
                    }
...
public:
    ...
    void write_stats_to_db(const string& database_name) const {
...
int main(int argc, char* argv[]) {
    try {
        ...
        reader.close();
        const string database_name{"stats.db"};
        vout << "Writing statistics to DB '" << database_name << "'...\n";
