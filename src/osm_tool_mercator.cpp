...
class OutputOGR : public osmium::handler::Handler {
    ...
    static void print_area_error(const osmium::Area& area, const osmium::geometry_error& e) {
        cerr << "Ignoring illegal geometry for area "
                  << area.id()
                  << " created from "
...
public:
    ...
    void area(const osmium::Area& area) {
        try {
            ...
            gdalcpp::Feature feature{m_layer_multipolygons, move(geom)};
...
void print_help() {
    cout << "oat_mercator [OPTIONS] OSMFILE\n\n"
...
int main(int argc, char* argv[]) {
    try {
        ...
        while (true) {
            ...
            switch (c) {
                case 'f':
                    ...
                    break;
                case 'd':
                    assembler_config.debug_level = optarg ? atoi(optarg) : 1;
                    break;
                case 'i':
        ...
        unique_ptr<osmium::area::ProblemReporterOGR> reporter;

        if (report_problems) {
            reporter = make_unique<osmium::area::ProblemReporterOGR>(dataset);
        }
        ...
        vout << "reading nodes and ways and assembling areas\n";
