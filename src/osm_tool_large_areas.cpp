...
class LargeAreasHandler : public osmium::handler::Handler {
    ...
    static pair<const char*, const char*> get_subtype(const osmium::TagList& tags) {
        for (const osmium::Tag& tag : tags) {
            if (!strcmp(tag.key(), "boundary")  ||
                ...
                return make_pair(tag.key(), tag.value());
            }
        }
        return make_pair("", "");
    }
...
static void print_help() {
    cout << "oat_large_areas [OPTIONS] OSMFILE\n\n"
